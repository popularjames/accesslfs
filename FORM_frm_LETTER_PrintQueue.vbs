...
'''  HISTORY:
'''  =====================================
'''  - 4/13/2016: KD: Added code to set view field codes to false in old letter tool
...
Private coParentFrm As Form_frm_LETTER_Main
...
Public Property Get SelectedContractId() As Long
    If clContractId = 0 Then
        If coParentFrm Is Nothing Then
            Set coParentFrm = Me.Parent.Form
        End If
        clContractId = coParentFrm.ContractId
    End If
...
'    If TypeName(Me.lstQueue.RecordSet) = "Recordset2" Then
'Stop
'        Set oDaoRs = Me.lstQueue.RecordSet
''        Set cdctQueueColumns = getdaofieldordinalposition(oDaoRs)
'        GoTo Block_Exit
'    End If
...
Dim sOrigPrinter As String
    ...
    ' Print letter
    If GenerateLetters(fmrStatus, bAtLeastOneErrored) = False Then
...
Dim MyAdo As clsADO
    ...
    If Me.SelectedContractId <> 0 Then
        strSQLcmd = strSQLcmd & " AND ContractId = " & CStr(Me.SelectedContractId)
    Else
        Stop    ' Not allow to do both! Fight me on this issue!
    End If
    ...
    Do While Not rsLetterTemplate.EOF
        With rsLetterTemplate
            ...
            objLetterInfo.ContractId = ![ContractId]
            ...
            If FileExists(strLocalTemplate) = True Then
                Call DeleteFile(strLocalTemplate, False)
            End If
    ...
    'TL add account ID logic
    If Me.SelectedContractId > 0 Then
Stop    '' KD Comeback - we have a function for this - duplicated code!!!
        Set rsLetterConfig = db.OpenRecordSet("select * from LETTER_Config where AccountID = " & gintAccountID & " AND ContractId = " & CStr(Me.SelectedContractId))
    
    Else
        Stop ' not allowed to do this for letter generation - fight me on this issue!!!
    End If
...
'    Dim MyRecordset As dao.RecordSet
    ...
    For Each varItem In lstQueue.ItemsSelected
        If Me.lstQueue.Column(MyRecordset.Fields("Status").OrdinalPosition, varItem) = "w" Or _
...
'    cmd.Parameters.Append cmd.CreateParameter("ErrMsg", adChar, adParamOutput, 255, "")
    ...
    For Each oLetter In cdctSelectedLetters.Letters
        ...
        If UCase(strStatus) = "P" Or UCase(strStatus) = "G" Or UCase(strStatus) = "W" Then
...
'
'
'    sFrom = " FROM CMS_AUDITORS_CODE.dbo.v_LETTER_Work_Queue Q LEFT JOIN ( SELECT D.InstanceId, Max(LetterBatchId) AS BatchId FROM LETTER_Static_Details D GROUP BY D.InstanceID ) SD " & _
'            " ON Q.InstanceId = SD.InstanceID "
    ...
    Select Case cboViewType
        ...
        Case "View Un-Processed Letters"
            Dim sYesterday As String
                sYesterday = Format(DateAdd("d", -1, Now()), "mm/dd/yyyy")
                
            sWhere = " WHERE (Status In ('W','R','C' ) OR (Status = 'G' AND RowCreateDt < #" & sYesterday & "# )) "
            
'            Me.cmbStatus.RowSource = "W;Ready to be Generated;R;Re-Generate;G;Generated but not printed"
            
        Case "View Processed Letters"
'           sWhere = sWhere & " AND Status IN ('P','G') "
'           sWhere = " WHERE Status IN ('P', 'G') AND LetterType <> 'REMINDADR' "
           sWhere = " WHERE Status IN ('P', 'G') "
'            Me.cmbStatus.RowSource = "R;Re-Generate;G;Generated but not printed"
'            sWhere = " WHERE ((Status = 'P' AND ProcessedDt >= #" & Nz(txtFromDate.Value, "01/01/1900") & "# and ProcessedDt < #" & _
'                Format(dtThrouDt, "mm-dd-yyyy") & "# " & ") OR (Status = 'G' AND LetterType <> 'REMINDADR' ))"
            sWhere = " WHERE ((Status = 'P' AND ProcessedDt >= #" & Nz(txtFromDate.Value, "01/01/1900") & "# and ProcessedDt < #" & _
                Format(dtThrouDt, "mm-dd-yyyy") & "# " & ") OR (Status = 'G' AND LetterType <> 'REMINDADR' ))"
    ...
    If coParentFrm Is Nothing Then Set coParentFrm = Me.Parent.Form
    

    If coParentFrm.ContractId <> 0 Then
        Me.SelectedContractId = coParentFrm.ContractId
        sWhere = sWhere & " AND Q.Contractid = " & coParentFrm.ContractId
    Else
'        If Me.visible = False Then
'            GoTo Block_Exit
'        End If
'        Stop    ' not allow to do more than 1 contract at a time!
    End If
...
'' Purpose of this next function is to see if there are any UN Processed
'' letters that are Not Shown due to being by a different date
'' which is a very common confusion issue
Public Function CheckForAlerts() As Boolean
On Error GoTo Block_Err
Dim strProcName As String
Dim oAdo As clsADO
Dim oRs As ADODB.RecordSet

    strProcName = ClassName & ".CheckForAlerts"
    ...
    With oAdo
        ...
        .sqlString = "usp_LETTER_Automation_Check_For_Queue_Alert"
        .Parameters.Refresh
        .Parameters("@pAccountId") = CStr(gintAccountID)
        .Parameters("@pFromDate") = CDate(Nz(txtFromDate.Value, "1/1/1900"))
        Set oRs = .ExecuteRS
        If .GotData = False Then
            Me.lblAlert.visible = False
        Else
            Me.lblAlert.ControlTipText = ""
            Me.lblAlert.Tag = ""
            Me.lblAlert.Caption = "ALERT: There are Un-processed Letters from a different date!"
            While Not oRs.EOF
                'Me.lblAlert.Caption = Me.lblAlert.Caption & oRs("RowCreateDt").value & ", "
'                Me.lblAlert.ControlTipText = Me.lblAlert.ControlTipText & oRs("RowCreateDt").value & ", "
                Me.lblAlert.Tag = Me.lblAlert.Tag & oRs("RowCreateDt").Value & ", "
                oRs.MoveNext
            Wend
            Me.lblAlert.ControlTipText = left(Me.lblAlert.Tag, 255)
...
Dim sStatusStr As String
    ...
    If Me.FilteredByGenerated = True Or Me.FilteredByErrors = True Then
        ...
        Set cdctSelectedLetters = GetSelectedItems(True, "GE", , sErrMsg, sStatusStr)
        If cdctSelectedLetters Is Nothing Then
            iTotalSelected = 0
        Else
        ...
        If InStr(1, sStatusStr, "|P|", vbTextCompare) > 0 Then
            If MsgBox("You have some letters selected with a status of 'P' which means that" & vbCrLf & "this was already done. Are you sure you want to proceed?" & vbCrLf & "Ok to continue, Cancel to cancel", vbOKCancel, "Already printed letters selected - Continue?") = vbCancel Then
                LogMessage strProcName, "WARNING", "User canceled due to picking some that had already been printed!"
                GoTo Block_Exit
            End If
            LogMessage strProcName, "WARNING", "User did not cancel and has selected some letters in 'P' status already"
        End If
...
Private Function GetFolderFileCounts(ByVal sStartFolder As String, dctFiles As Scripting.Dictionary, Optional bRecurse As Boolean = True) As Boolean
On Error GoTo Block_Err
Dim strProcName As String
Dim oFso As Scripting.FileSystemObject
Dim oFldr As Scripting.Folder
Dim osFldr As Scripting.Folder


    strProcName = ClassName & ".GetFolderFileCounts"
    While Right(sStartFolder, 1) = "\" And Len(sStartFolder) > 1
        sStartFolder = left(sStartFolder, Len(sStartFolder) - 1)
    Wend
    ...
    If sStartFolder = "" Then
        ...
        Stop
    End If
    sStartFolder = UCase(sStartFolder)
    
    Set oFso = New Scripting.FileSystemObject
    
    If oFso.FolderExists(sStartFolder) = False Then
        GoTo Block_Exit
    End If

    If dctFiles Is Nothing Then
        Set dctFiles = New Scripting.Dictionary
    End If

    If dctFiles.Exists(UCase(sStartFolder)) = False Then
        ' we don't want to update it if it's already in there this pass
        Set oFldr = oFso.GetFolder(sStartFolder)
        
        If bRecurse = True Then
            For Each osFldr In oFldr.SubFolders
                Call GetFolderFileCounts(osFldr.Path, dctFiles, bRecurse)
            Next
        End If
        
        dctFiles.Add sStartFolder, oFldr.Files.Count
    End If

    GetFolderFileCounts = True
...
Dim lFileFolderCount As Long
Dim dctFolderFileCounts As Scripting.Dictionary
...
'Stop
    ...
    sSql = "SELECT DISTINCT X.InstanceID, X.LetterType, LetterName As DocumentPath, PageCount, ProvName, ContactTitle " & _
            ", ContactName, ContactType, Addr01, Addr02, Addr03, City, State, Zip, SD.LetterBatchId " & _
...
Dim lLastBatchId As Long
Dim lThisBatchid As Long
    ...
    With oAdo
        ...
        End If
    End With
sTempFldr = QualifyFldrPath(GetSetting("MAILROOM_LETTER_PATH")) & Format(Now(), "yyyy-mm-dd") & "\"
'Stop
    Call CreateFolders(sTempFldr)
    
    Set oFso = New Scripting.FileSystemObject
    Set oFldr = oFso.GetFolder(sTempFldr)
    lFileFolderCount = oFldr.Files.Count
    Set oFldr = Nothing
...
'    End If
'
'    Call DeleteFullFolder(sTempFldr, False)
    
    Call GetFolderFileCounts(sTempFldr, dctFolderFileCounts, False)
    ...
    lngProgressCount = oRs.recordCount  ' (times 2 because copying is step 1, then combining is step2)
...
Dim sSaveInThisFldr As String
    ...
    While Not oRs.EOF
        ...
        lCnt = lCnt + 1
        sNewStatus = "P"
        sCurInstanceId = oRs("InstanceId").Value
        lThisBatchid = oRs("LetterBatchID").Value
        
        sSaveInThisFldr = QualifyFldrPath(sTempFldr) & UCase(oRs("LetterType").Value)
        Call CreateFolders(sSaveInThisFldr)
        
        Call GetFolderFileCounts(sSaveInThisFldr, dctFolderFileCounts, False)
        If dctFolderFileCounts.Exists(sSaveInThisFldr) = True Then
            lFileFolderCount = CLng(dctFolderFileCounts.Item(sSaveInThisFldr))
        Else
            lFileFolderCount = 0
        End If
        sSaveInThisFldr = sSaveInThisFldr & "\"
        ...
        If FileExists(sSaveInThisFldr & SFileName) = True Then
            ...
            If FileExists(sOrigFilePath) = False Then
                ...
                GoTo NextRow
            Else
                If DeleteFile(sSaveInThisFldr & SFileName, False) = False Then
                    LogMessage strProcName, "WARNING", "The file being copied already exists and appears to be locked open", sSaveInThisFldr & SFileName
                    Call ErrorCallStack_Add(0, "The file being copied already exists and appears to be locked open.", strProcName, sSaveInThisFldr & SFileName, , , oRs("InstanceID").Value, oRs("LetterType").Value)
                    GoTo NextRow
                End If
            End If
        Else
'        Stop
        ...
        SFileName = Format(lCnt + lFileFolderCount, "0000") & "_" & SFileName
        
        If CopyFile(sOrigFilePath, sSaveInThisFldr & SFileName, False, strErrMsg) = False Then
            LogMessage strProcName, "ERROR", "There was a problem copying a file to the temp folder: " & strErrMsg, sOrigFilePath & " to " & sSaveInThisFldr & SFileName
            Call ErrorCallStack_Add(0, "There was a problem copying a file to the temp folder: " & strErrMsg, strProcName, sOrigFilePath & " to " & sSaveInThisFldr & SFileName, , , oRs("InstanceID").Value, oRs("LetterType").Value)
            iErrCnt = iErrCnt + 1
        End If
        ...
        oCmd.Parameters("@pDocNum").Value = lCnt
        oCmd.Execute
        If Nz(oCmd.Parameters("@ErrMsg").Value, "") <> "" Then
            LogMessage strProcName, "WARNING", "There was a problem changing the letters status : " & oCmd.CommandText & " " & sCurInstanceId, sCurInstanceId & " E: " & Nz(oCmd.Parameters("@ErrMsg").Value, "")
'            Stop
        End If
        Set oCmd = Nothing
...
'        oRs.MoveNext
        ...
        End If
NextRow:
'        If lLastBatchId <> lThisBatchid Then
'            Call InsertBatchDetails(lThisBatchid, sSaveInThisFldr)
'        End If
        lLastBatchId = lThisBatchid
...
Private Function InsertBatchDetails(lThisBatchid As Long, sSaveInThisFldr As String) As Boolean
On Error GoTo Block_Err
Dim strProcName As String
Dim oAdo As clsADO
    ...
    With oAdo
        .ConnectionString = GetConnectString("v_Code_Database")
        .SQLTextType = StoredProc
        .sqlString = "usp_LETTER_AddBatchDetails"
        .Parameters.Refresh
        .Parameters("@pBatchid") = lThisBatchid
        .Parameters("@pSaveInThisFldr") = sSaveInThisFldr
        .Execute
...
'
'Private Function PrintGeneratedLetters_LEGACY(oStatusForm As Form_ScrStatus) As Boolean
'On Error GoTo Block_Err
'Dim strProcName As String
'Dim oAdo As clsADO
'Dim oRs As ADODB.RecordSet
...
'    bForward = False
'
'    strProcName = ClassName & ".PrintGeneratedLetters_LEGACY"
''Stop
'    '' 20130815 KD: All we have to do here is:
'    '' - get a RS with the ones to select including the document paths (Sorted by pagecount ASC)
'
...
'    Shell "explorer.exe " & Chr$(34) & sTempFldr & Chr$(34), vbNormalFocus
'
'    PrintGeneratedLetters_LEGACY = True
'    RefreshMain
'
'Block_Exit:
'    If Not oCn Is Nothing Then
...
Dim oCn As ADODB.Connection
    ...
    lTranReturn = oCn.BeginTrans


'    'TGH UPdated this has been added in for previewing letters.
    If lTotalToPrint > 0 And bPreviewViewAllowed Then
        If Not PreviewViewLetters(oCn, lTotalToPrint, fmrStatus) Then
            LogMessage strProcName, "ERROR", "There was an error in PreviewViewLetters for at least 1 letter!"
            oCn.RollbackTrans
            lTranReturn = 0
...
Block_Exit:
    ...
    Exit Function
Block_Err:
    LogMessage TypeName(Me) & ".cmdViewLetters_Click", "ERROR", "Letter system error: " & Err.Description
    If lTranReturn > 0 Then
        oCn.RollbackTrans
        lTranReturn = 0
    End If
...
Dim oRs As ADODB.RecordSet
    ...
    With oAdo
        ...
        .Parameters("@pAccountId") = IIf(gintAccountID = 0, 1, gintAccountID)
            ' Not allowed to do more than 1 contract at a time
        .Parameters("@pContractId") = Me.SelectedContractId
        Set oRs = .ExecuteRS
        If .GotData = False Then
...
Dim oWordDoc As Word.Document
 ...
 Stop
 '  Only time we should get here is when more than 1 contractid is in our selected set.. but we aren't allowing more than 1 contract at a time for letters
    ...
    For Each oLetterToGenerate In cdctSelectedLetters.Letters
        ...
        If CopyTemplates(dctLetterTemplate, strLetterType, oLetterToGenerate.ContractId) = False Then
        ...
        If IsEmpty(dctLetterTemplate.Item(UCase(strLetterType))) Then
            Stop
        End If
        Set objLetterTemplate = dctLetterTemplate.Item(UCase(strLetterType))
        ...
        If fmrStatus.EvalStatus(2) = True Then
            ...
            GoTo Block_Exit
        End If
        ' I wonder how much better it would be if we only use a single MS Word.Application instance instead of spawning it each time?
        ' Let's do that, but here, before we call PrintLetterInstance let's make damn sure that no other documents are already open
        ...
        If PrintLetterInstance(oLetterToGenerate, objLetterTemplate.TemplateLoc, strOutputFileName, strOutputLocation, strProvNum, _
                                        strODCFile, strLetterType, iPageCount, oWordApp) = False Then
'            Stop
...
Private Function PrintLetterInstance(oLetterInst As clsLetterInstance, pstrTemplateName As String, _
            ...
            Optional iPageCount As Integer, Optional objWordApp As Word.Application) As Boolean
On Error GoTo Block_Err
Dim strProcName As String
Dim oAdo As clsADO
Dim oCn As ADODB.Connection
...
Dim objLetterInfo As clsLetterTemplate
Dim objWordDoc As Word.Document, _
    objWordMergedDoc As Word.Document
Dim bAppPassed As Boolean
Dim sMergeSproc As String
    ...
    ''  "go live" with it.
    '' Note: do not need ContractId here as each instanceid can only be 1 contract
...
''    pstrOutputFileName = Replace(pstrOutputFileName, "\\ccaintranet.com\dfs-cms-fld\Audits\CMS\LETTER_REPOSITORY\LETTERS\", "\\ccaintranet.com\dfs-cms-ds\Data\CMS\AnalystFolders\KevinD\QR_Codes\pdf_test")
    ...
    If FileExists(pstrOutputFileName) = True Then
...
'    oAdo.BeginTrans
    ...
    ' update LETTER status
    ' Do not need contract id here because each instance id is only 1 contract
    Set oCmd = New ADODB.Command
    'oCmd.ActiveConnection = oAdo.CurrentConnection
...
Private Function CopyTemplates(dctTemplatesDict As Scripting.Dictionary, strLetterType As String, Optional lContractId As Long) As Boolean
On Error GoTo Block_Err
Dim strProcName As String
...
'Dim lContractId As Long
    ...
    If dctTemplatesDict Is Nothing Then Set dctTemplatesDict = New Scripting.Dictionary
        ' no need to pull this from the db..
    If IsMissing(lContractId) Then
        lContractId = Me.SelectedContractId
    End If


    strSQL = "SELECT LetterType, TemplateLoc FROM LETTER_Type WHERE (AccountID = " & CStr(gintAccountID) & " OR " & CStr(gintAccountID) & " = 0 ) and LetterType = '" & strLetterType & "' " & _
        " AND ContractId = " & CStr(lContractId)
...
'            End If
            ...
            objLetterInfo.ContractId = lContractId
            
            If dctTemplatesDict.Exists(UCase(rsLetterTemplate("LetterType").Value)) = True Then
                Set dctTemplatesDict.Item(UCase(rsLetterTemplate("LetterType").Value)) = objLetterInfo
            Else
                dctTemplatesDict.Add UCase(rsLetterTemplate("LetterType").Value), objLetterInfo
            End If
...
Private Function GetSelectedItems(Optional bViewOnly As Boolean = False, Optional sDesiredStatus As String, _
            Optional sDisallowedStatus As String, Optional sErrMsg As String, Optional sStatusStr As String) As clsLetterInstanceDct
On Error GoTo Block_Err
Dim strProcName As String
...
'Stop    ' Kev: why did they user CnlyProvId and call it ProvNum when there's a ProvNum in the recordset too??
        ...
        lContractId = UCase(Trim(Me.lstQueue.Column(dctPos.Item("CONTRACTID"), varItem)))
        ...
        If bOk = True Then
            ...
            ' build a string we can use to Instr() later to see if a particular status was selected
            If InStr(1, sStatusStr, sQStatus, vbTextCompare) < 1 Then
                If sStatusStr = "" Then
                    sStatusStr = "|" & sQStatus & "|"
                Else
                    sStatusStr = sStatusStr & sQStatus & "|"
                End If
            End If
            Call oLetters.AddLetterInstance(sLetterInstance, sQStatus, sProvNum, sLetterType, lContractId)
        End If
        bOk = False ' reset
    Next varItem
...
Stop
        lContractId = Me.lstQueue.Column(oRs.Fields("ContractId").OrdinalPosition, varItem)
...
Dim oAdo As clsADO
    ...
    ' Note: do not need contract id here because BatchId is unique and can never refer to more than 1 contractid
    Set oAdo = New clsADO
    With oAdo
        .ConnectionString = GetConnectString("v_Code_Database")
        .SQLTextType = StoredProc
