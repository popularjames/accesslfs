...
Private cdtcAccountIds As Scripting.Dictionary
...
Private Const cl_TimerInterval As Long = 300000
Private cbRefreshOnTimer As Boolean
...
Public Property Get SelectedAccountId() As Long
    SelectedAccountId = cmbBusiness.Value
...
' This needs to be redone.. It's too specific' KD COMEBACK
...
Dim iColCnt As Integer
    ...
    If oListV.ListItems.Count = 0 Then
        ' nothing to highlight - skip
        GoTo Block_Exit
    End If
    Set oLI = oListV.ListItems(1)

'' old way before my object
''    For Each oCHdr In oListV.ColumnHeaders
''        If sColName <> "" Then
''            If LCase(oCHdr.Text) = LCase(sColName) Then
''                bFoundCol = True
''                iCountDownCol = iColCnt
''            End If
''        Else
...
''        iColCnt = iColCnt + 1
''    Next
    iStatusCol = Nz(Me.QueueColumns.GetDetails("QUEUE", "ErrorDesc"), 0)
    iLetterTypeCol = Nz(Me.QueueColumns.GetDetails("QUEUE", "LetterType"), -1)
    iCountDownCol = Nz(Me.QueueColumns.GetDetails("QUEUE", sCOLUMNNAME), -1)
    iPullCol = Nz(Me.QueueColumns.GetDetails("QUEUE", sPULLCOLNAME), -1)
    If iStatusCol = 0 Then
        iStatusCol = iCountDownCol
    End If
'    If bFoundCol = False Then GoTo Block_Exit
    
    For Each oLI In oListV.ListItems
        ' First, countdown column
        If iCountDownCol >= 0 Then
            If IsNumeric(oLI.SubItems(iCountDownCol)) Then
                If CInt(oLI.SubItems(iCountDownCol)) <= Me.DaysLeftToSendForHighlight Then
                Set oLetterType = New clsLetterType
                ...
                End If
            End If
            ElseIf IsDate(oLI.SubItems(iCountDownCol)) Then
                ' in case it's a date (shouldn't be but..)
                Debug.Print DateDiff("d", CDate(oLI.SubItems(iCountDownCol)), Now())
                If DateDiff("d", CDate(oLI.SubItems(iCountDownCol)), Now()) < 2 Then
                Set oLetterType = New clsLetterType
                If iLetterTypeCol > 0 Then
                    oLetterType.LetterType = oLI.SubItems(iLetterTypeCol)
                Else
            ...
            End If
        End If
        ' now,  status column, lettertype, and pull columns
            ' status : error
        If InStr(1, Nz(oLI.SubItems(iStatusCol), ""), "E", vbTextCompare) > 0 Then
            ' error:
                iRet = iRet + 1
                oLI.Bold = True
                oLI.ForeColor = lErrColor
                oLI.ToolTipText = "!!! This batch had an error occur !!!"
        End If
        If Nz(oLI.SubItems(iPullCol), "") <> "" Then
                ' SOmething should be pulled: maybe the whole batch, maybe just a single letter...
            iRet = iRet + 1
            oLI.Bold = True
            oLI.ForeColor = lErrColor
            oLI.EnsureVisible
'            oLI.Selected = True
'            oLI.Checked = True
            oLI.ToolTipText = "!!! This Batch has letter(s) that need to be pulled! "
'            bSomethingToPull = True
        End If
    Next
        
    ' I don't have to do this here - it's going to happen on the form's timer (so in case the dataset
    ' isn't refreshed we still get notified)
'    If bSomethingToPull = True Then
...
'    iBatchTypeCol = QueueColumns.GetDetails("QUEUE", "BatchType")
    ...
    For Each oLItem In oLV.ListItems
        If oLItem.Checked = True Then
            ...
            lBackEndBatchId = QueueColumns.GetLiValue(oLItem, "QUEUE", "BackEndBatchId")
                ' now we want something like this:
                ' "BatchId=123;BatchType=Normal"
            sThisLine = MakeXmlString("MailBatchid=" & lBackEndBatchId & ";BatchType=" & sBatchType, "row", , , True)
            ...
            If QueueColumns.GetLiValue(oLItem, "QUEUE", "JobIsDuplex") = "Y" Then
                bDuplex = True
            End If
            If QueueColumns.GetLiValue(oLItem, "QUEUE", "RequestedToPull") = "Y" Then
                sMsg = "At least 1 item that you selected to print is requested to be pulled - we will not print it therefore we are canceling this job!"
                LogMessage strProcName, "SYSTEM CONTROL", sMsg, , True
                Call GenerateAndDisplayPullMessage
                Stop
                GoTo Block_Exit
            End If
    ...
    If sSelectedPrinter = "" Then
        LogMessage strProcName, , "User canceled at the printer selection dialog"
        GoTo Block_Exit
    End If
    DoCmd.Hourglass True
    ...
    With oAdo
        ...
        .sqlString = "usp_LETTER_Automation_MAILOPS_StartPrintingMult"
        .Parameters.Refresh
        ...
        If Nz(.Parameters("@pErrMsg").Value, "") <> "" Then
            Stop
            LogMessage strProcName, "ERROR", "Error in " & .sqlString, .Parameters("@pErrMsg").Value, True
        End If
...
Dim lAccountId As Long
    ...
    While Not oRs.EOF
        ...
        lAccountId = oRs("AccountId").Value
        sBatchType = oRs("BatchType").Value
        
        If CreateCoverPageAndPrint(lAccountId, lOpsBatchId, sBatchType, oRs("CombinedFilePath").Value, lBatchRowIdToClose, sSelectedPrinter, oFrmStatus) = False Then
...
'    DoCmd.OpenForm "frm_ALERT_NOTE", acNormal, , , , acHidden
    ...
    LoadMailRoomData
    Call GenerateAndDisplayPullMessage
    Me.TimerInterval = cl_TimerInterval
...
Dim lOrigInterval As Long
    ...
    Call coLVColPos.SetDetails("ERRORS", Me.lvQErrorDetails)
    Call RefreshData
    ' Check to see if there is a message from Business Operations
    ' to pull a letter or whatnot..
    Call GenerateAndDisplayPullMessage
    If cbRefreshOnTimer = True And lOrigInterval = 30000 Then
        Me.TimerInterval = cl_TimerInterval
    Else
    Me.TimerInterval = lOrigInterval
    End If
End Sub
Private Sub fraAccountList_AfterUpdate()
On Error GoTo Block_Err
Dim strProcName As String
    strProcName = ClassName & ".fraAccountList_AfterUpdate"
    Call PopulateAccountList
    If Me.cmbBusiness = "" Then
        Me.cmbBusiness = "ALL"
    End If
    Call RefreshData
Block_Exit:
...
Dim oLI As ListItem
    ...
    Call SetUpContextMenu("MailQueueErrors")
...
Dim iPullLevel As MailLevels
    ...
    Me.SelectedAccountId = CInt("0" & QueueColumns.GetLiValue(oLI, oLI.Tag, "AccountID"))
...
'    sCriteria = sCriteria & " AND OpsLetterBatchId = '" & oLI.Text & "'"
    ...
    sCriteria = "LetterType = '" & QueueColumns.GetLiValue(oLI, oLI.Tag, "LetterType") & "' "
    sCriteria = sCriteria & " AND AccountId = " & CStr(Me.SelectedAccountId)
    sCriteria = sCriteria & " AND LetterReqDt = '" & QueueColumns.GetLiValue(oLI, oLI.Tag, "LetterReqDt") & "'"
    
    sCriteria = sCriteria & " AND OpsLetterBatchId = '" & QueueColumns.GetLiValue(oLI, oLI.Tag, "OpsLetterBatchId") & "'"
        
    If QueueColumns.GetLiValue(oLI, "QUEUE", "RequestedToPull") = "Y" Then
        sCriteria = sCriteria & " AND (RequestedToPull = 'Y') "
    End If
...
Dim oLI As ListItem
    ...
    Call SetUpContextMenu("MailQueue")
...
GoTo Block_Exit
    ...
    With oAdo
        ...
        .Parameters.Refresh
        .Parameters("@pAccountType") = 1    'Me.fraAccountList    ' 1 = MCR, 2 = CRS, 0will be all (this will be the default because it's used elsewhere)
        Set oRs = .ExecuteRS
        If Nz(.Parameters("@pErrMsg").Value, "") <> "" Then
        ...
        End If
    End With
    Me.cmbBusiness.ColumnCount = 2
    Me.cmbBusiness.BoundColumn = 1
    Me.cmbBusiness.ColumnWidths = "0;2"
    Set Me.cmbBusiness.RecordSet = oRs
    Set cdtcAccountIds = New Scripting.Dictionary
    Dim oRsClone As ADODB.RecordSet
    Set oRsClone = oRs.Clone
    While Not oRsClone.EOF
        If cdtcAccountIds.Exists(oRsClone("AccountId")) = False Then
            cdtcAccountIds.Add CStr(oRsClone("AccountId").Value), 1
        End If
        oRsClone.MoveNext
    Wend
Block_Exit:
...
Dim oRs As ADODB.RecordSet
    ...
    With oAdo
        ...
        .Parameters("@pAccountType") = 1    'Me.fraAccountList
        Set oRs = .ExecuteRS
        If Nz(.Parameters("@pErrMsg").Value, "") <> "" Then
