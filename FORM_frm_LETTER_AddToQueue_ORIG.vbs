...
'    CurrentDb.Execute (Sstr)
    ...
    Sstr = "SELECT a.*, '" & Identity.UserName & "' as UserName FROM " & IIf(left(sFrom, 2) = "v_", "cms_auditors_code.dbo." & sFrom, "cms_auditors_claims.dbo." & sFrom) & " a " & IIf(msAdvancedFilter <> "", " WHERE " & msAdvancedFilter, "")
...
'
    ...
    For listloop = 1 To Me.lstSenders.ListCount
        If Me.lstSenders.ItemData(listloop) = Identity.UserName() Then
            ...
            Me.lstSenders.ListIndex = listloop - 1 'JS 07/01/2013
            Exit For
        End If
    Next
