...
Private coRuleDetails As clsBOLD_LetterRuleItemDetails
...
Public Property Get ItemDetails() As clsBOLD_LetterRuleItemDetails
    Set ItemDetails = coRuleDetails
End Property
Public Property Let ItemDetails(oItemDetails As clsBOLD_LetterRuleItemDetails)
    Set coRuleDetails = oItemDetails
End Property
...
'Stop
' It's friday and it's time to get out of here so I'm going to do so.. Hopefully I'll get drunk this weekend
' and flush all of the crap out of my head so I can start fresh on Monday!
...
Dim lLocalId As Long
    ...
    For iLItemIdx = oFromLv.ListItems.Count To 1 Step -1
        ...
        If oLItemFrom.Selected = True Then
            ...
            If coRuleDetails.ItemExists(oLItemFrom.Text, sUniqueInstance) = False Then
                oRuleItem.SecureLocalId (sUniqueInstance)
            Else
'                Stop
                lLocalId = GetLocalIdFromItemTypeId(oLItemFrom.Tag)
                
                Call oRuleItem.LoadFromId(lLocalId)
...
'            Call oPartnerLv.ColumnHeaders.Add(, oFld.Name, oFld.Name, lWidth)
    ...
    While Not oRs.EOF
        ...
        oLItm.Tag = oRs(IdColumnName).Value
...
Public Function MoveItemDetailToSelected(oDtl As clsBOLD_LetterRuleItemDetail) As Boolean
On Error GoTo Block_Err
Dim strProcName As String
Dim oLV As Object
Dim oLI As ListItem
    ...
    strProcName = ClassName & ".MoveItemDetailToSelected"
    
    Set oLV = Me.lstvUnSelected
    
    For Each oLI In oLV.ListItems
'Stop
        If oLI.Text = oDtl.ItemName Then
            oLI.Selected = True
        Else
            oLI.Selected = False
        End If
...
Private Function GetLocalIdFromItemTypeId(lItemTypeId As Long) As Long
On Error GoTo Block_Err
Dim strProcName As String
Dim oDb As DAO.Database
Dim oRs As DAO.RecordSet

    strProcName = ClassName & ".GetLocalIdFromItemTypeId"
    Set oDb = CurrentDb
    Set oRs = oDb.OpenRecordSet("SELECT LocalId FROM " & cs_TEMP_RULE_TABLE_NAME & " WHERE ItemType = '" & Me.ItemType & "' AND LkupId = " & CStr(lItemTypeId))
    
    If oRs.EOF And oRs.BOF Then
        GetLocalIdFromItemTypeId = 0
    Else
        GetLocalIdFromItemTypeId = oRs("LocalId").Value
        oRs.Close
    End If
Block_Exit:
    Set oRs = Nothing
