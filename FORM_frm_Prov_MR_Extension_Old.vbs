...
Private Sub cboReason_AfterUpdate()
    ...
    If cboReason.Value = "Other" Then
        ...
        cboDaysExtend.Enabled = True
    Else
        txtOtherReason.Enabled = False
        cboDaysExtend.Enabled = False
    End If
...
Private Sub cmdSave_Click()
    ...
    If lstSelectedClaims.ListCount > 1 Then
        ...
        Dim i As Integer
        For i = 1 To lstSelectedClaims.ListCount - 1
            'MsgBox lstSelectedClaims.Column(i, 1)
            'MsgBox lstSelectedClaims.Column(0, i)
            'MsgBox i
...
Function claimsLookup(searchField As String, searchValue As String, excludeMRReceived As String)
    ...
    cmd.ActiveConnection = MyCodeAdo.CurrentConnection
    cmd.commandType = adCmdStoredProc
    cmd.CommandText = "usp_PROV_MR_Extension_Claims_Lookup"
    cmd.Parameters.Refresh
...
On Error GoTo ErrHandler
    ...
    strSQL = "SELECT RequestNumber,CnlyClaimNum FROM PROV_MR_Extension_Lookup WHERE sessionID='" & Identity.UserName & "'"
    Set db = CurrentDb
