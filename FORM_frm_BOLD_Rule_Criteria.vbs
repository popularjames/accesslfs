...
Private Const cs_TEMP_TABLE_NAME As String = "tmp_BOLD_LETTER_Rules"
...
'            With coCurrentItem
'                sSql = "INSERT INTO " & cs_TEMP_TABLE_NAME & " (LocalId, ItemType, RemoteId, Boolean, LkupId, LkupDisplay, ItemName, RelatedFieldName, Operator, Value) "
'                sSql = sSql & " VALUES (" & CStr(.ID) & ", " & .ItemType & ", " & CStr(.RuleId) & ", " & .BooleanVal & ", "
'                sSql = sSql & .LookupId & ", " & .LookupDisplay & ", " & .ItemName & ", " & .ItemFieldName & ", " & .Operator & ", " & .ItemValue & ")"
...
'        Stop    ' need to populate the coCurrentItems object FROM the table (should probably just have a method there since it should really always be attached to that table
                ...
                Me.txtValue.ControlSource = "Value"
...
'                coCurrentItem.LoadFromID (Me.txtLocalId)
