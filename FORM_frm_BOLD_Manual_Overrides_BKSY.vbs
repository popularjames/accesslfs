...
Dim iInt As Integer
    ...
    With oAdo
        ...
        .sqlString = "usp_LETTER_Automation_ManualFiltersList"
        .Parameters.Refresh
...
Dim oAdo As clsADO
Dim lNewId As Long
Dim oFrm As Form_frm_BOLD_Manual_Overrides_Edit

    strProcName = ClassName & ".cmdAddNew_Click"
    
    sName = InputBox("Name of new filter:", "Name filter")
    If Trim(sName) = "" Then
        ' canceled
        GoTo Block_Exit
    End If
    ...
    With oAdo
        .ConnectionString = CodeConnString
        .SQLTextType = StoredProc
        .sqlString = "usp_LETTER_Automation_ManFilterAdd"
        .Parameters.Refresh
        .Parameters("@pAccountId") = gintAccountID
        .Parameters("@pFilterName") = sName
        .Execute
        If Nz(.Parameters("@pErrMsg").Value, "") <> "" Then
            Stop
        End If
    ...
    If coManFilter.LoadFromId(lNewId) = False Then
        Stop
    End If
'Stop
    
    Set oFrm = New Form_frm_BOLD_Manual_Overrides_Edit
    oFrm.FilterObject = coManFilter
...
Dim oFrm As Form_frm_BOLD_Manual_Overrides_Edit
    ...
    Set coManFilter = New clsFilter
    If coManFilter.LoadFromId(lFilterId) = False Then
        Stop
    End If
