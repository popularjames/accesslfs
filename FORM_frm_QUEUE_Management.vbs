...
Option Explicit
'=============================================
' ID:          Form_frm_QUEUE_Management
' Description:
'      Manage the Claim Queue and Exception data.
'
'
' Modification History:
'   2013-07-01 by BJD to Filter Financial Exceptions from the User Exception Display.
'
'
' =============================================
...
'AWC - confirm reassignment when rationale exists, prevent if claim is locked
Private WithEvents frmConfirmReAssign As Form_frm_QUEUE_ReAssign_Confirm
Attribute frmConfirmReAssign.VB_VarHelpID = -1
Private mbConfirmReAssign As Boolean
Private mbAssignAll As Boolean
...
Const CstrFilterExceptionUser As String = " and  ExceptionType NOT LIKE 'FX%' " 'Filter from User Exception Display.
...
Private Sub cboDetail_Change()
    ...
    If Me.lstDetail.ListCount - 1 > 0 Then
        ColReSize.AutoSize
    End If
    Set ColReSize = Nothing  'kcf 7/26/2013
...
Dim strProcName As String
    ...
    strProcName = TypeName(Me) & ".cboHdr_Change"
    ...
    mCurrState.Hdr_SQL = SetQueueSQL(mCurrState.Hdr_ViewOrder)
        ...
        ' don't need the rowsource - we're using ADO
...
Dim dtStart As Date
    dtStart = Now
    oRs.Open mCurrState.Hdr_SQL, oCn
    Debug.Print ProcessTookHowLong(dtStart)
        

'    Set oRs = myCode_Ado.OpenRecordSet
    
    If oRs Is Nothing Then
        MsgBox "There was an error while executing the query!" & oCn.Errors.Item(0).Description
    Else
        Set lstHdr.RecordSet = oRs  ' .Clone    ' 20131009 KD: Weird behavior..
    End If
    
'    Debug.Print ProcessTookHowLong(dtStart)
'    Debug.Assert 1 <> 1
'
'    Set lstHdr.Recordset = oRs
...
Private Sub Form_Load()
    ...
    If IsSubForm(Me) Then
        ...
        Select Case UCase(Nz(Me.Parent.Parameter, "USER"))
            Case "USER", "MANAGER"
                mGroupName = "QUEUE_HDR"
            Case "USER_EXCEPTION", "MANAGER_EXCEPTION"
                mGroupName = "QUEUE_EXCEPTION"
            Case Else
                mGroupName = "QUEUE_HDR"
        End Select
    ...
    MyAdo.sqlString = "select * from QUEUE_Display_Config " & _
                      ...
                      " and AccountID = " & gintAccountID & _
                      " and ComboBoxOrder = 0" & _
                      " order by case when CtrlColumn = 'DaysToReview' then 0 else 1 end "
    ...
    If cboDetail.Column(0) = mstrDetailView Then
        ...
        If strStatus = "FORWARD" Then
            ...
            Else
                ...
                cmdReAssign.Enabled = False
            End If
            If mOperMode = OperationMode.Manager Then
                cmdReAssign.Enabled = True
            End If
            
        ElseIf strStatus = "OPEN" Or strStatus = "REPLY" Then
            If UCase(mstrUserName) = strAssignedTo Then
                cmdReAssign.Enabled = True
...
Dim lBkmark As Long
Dim ColResize1 As clsAutoSizeColumns

    If TypeOf Me.lstHdr.RecordSet Is ADODB.RecordSet Then
        lBkmark = Me.lstHdr.ListIndex   'Me.lstHdr.Recordset.Bookmark
    End If
    ...
    ColResize1.SetControl Me.lstHdr
    'don't resize if lstclaims is null
    If Me.lstHdr.ListCount > 1 Then
        ColResize1.AutoSize
    End If
    Set ColResize1 = Nothing

    If TypeOf Me.lstHdr.RecordSet Is ADODB.RecordSet Then
        Me.lstHdr.Selected(lBkmark + 1) = True
    End If
...
Private Function SetQueueSQL(QueueLevel As String) As String
    ...
    SQL.GroupBy = ""
    SQL.OrderBy = ""
    If mGroupName = "QUEUE_HDR" Then
        SQL.From = "from v_QUEUE_Hdr"
    Else
        SQL.From = "from v_QUEUE_Exception"
    End If
                ...
                Else
                    ...
                    While mrsAgeCalConfig.EOF <> True
                        ...
                        If UCase(mrsAgeCalConfig("CalcAge")) = "Y" Then
                            SQL.Select = SQL.Select & "(datediff(d," & mrsAgeCalConfig("CtrlColumn") & ",getdate())+1) as " & mrsAgeCalConfig("DispColName") & ", "
                            SQL.OrderBy = SQL.OrderBy & "(datediff(d," & mrsAgeCalConfig("CtrlColumn") & ",getdate())+1), "
                        ElseIf mrsAgeCalConfig("CtrlColumn") = "DaysToReview" Then
                            'do nothing, on purpose
'                            SQL.Select = SQL.Select & "(" & mrsAgeCalConfig("CtrlColumn") & ") as " & mrsAgeCalConfig("DispColName") & ", "
'                            SQL.OrderBy = SQL.OrderBy & "(" & mrsAgeCalConfig("CtrlColumn") & "), "
                        End If
    ...
    If Not bDetailView Then
        ...
        Else
            ...
            While mrsAgeCalConfig.EOF <> True
                ...
                ElseIf mrsAgeCalConfig("CtrlColumn") = "DaysToReview" Then
                    SQL.Select = SQL.Select & ", min(" & mrsAgeCalConfig("CtrlColumn") & ") as " & mrsAgeCalConfig("DispColName")
                End If
                mrsAgeCalConfig.MoveNext
            Wend
        End If
    ...
    If mOperMode = OperationMode.User Then
        ...
        If mrsDisplayConfig.EOF <> True Then
            ...
            End If
            'Filter the Financial Exceptions from the User Exception Queue display.
            If mGroupName = "QUEUE_EXCEPTION" Then
                 SQL.Where = SQL.Where & CstrFilterExceptionUser
            End If
...
Private Sub frmReAssignSelect_ReAssignQueue(AssignedTo As String, Comment As String, Action As String)
    ...
    'AWC - variables for lock and rationale check on reassign
    Dim strRationale As String
    Dim strLockUserID As String
    Dim rsReassignCheck As ADODB.RecordSet
    
    'AWC - switch to ignore rationale and reassign all remaining
    mbAssignAll = False
        ...
        Else
            ...
            Select Case UCase(Action)
                ...
                Case "REPLY"
                    ...
                    End If
                Case Else
                    If strStatus = "FORWARD" And mOperMode <> OperationMode.Manager Then
            ...
            ' Check if record is locked or rationale exists
            mbConfirmReAssign = True
            If UCase(Action) = "REASSIGN" Then
                ...
                If Len(strLockUserID) > 0 Then
                    MsgBox "Claim " & strCnlyClaimNum & " cannot be reassigned because it is locked for editing by " & strLockUserID & "."
                    mbConfirmReAssign = False
                
                'Something was entered in Rationale - give user choice
                ElseIf mbAssignAll = False And Len(strRationale) > 0 Then
                    'Show rationale and let user choose to continue, don't assign, or assign all
                    Set frmConfirmReAssign = New Form_frm_QUEUE_ReAssign_Confirm
                    frmConfirmReAssign.rationaleLabel.Caption = "Rationale exists for claim " & strCnlyClaimNum & "."
                    frmConfirmReAssign.rationaleText = strRationale
                    ShowFormAndWait frmConfirmReAssign
                    Set frmConfirmReAssign = Nothing
                End If
            End If
        
            ' update Queue header
            If mbConfirmReAssign = True Then
                ' update Queue header
...
Private Sub frmConfirmReAssign_ConfirmReAssign(Action As String)
    
    'Process selections user can make on reassignment when rationale exists
    Select Case Action
        
        'Reassign current claim
        Case "yes"
            mbConfirmReAssign = True
        
        'Don't reassign, and continue to next one
        Case "no"
            mbConfirmReAssign = False
        
        'Reassign all, and don't check for any other rationales
        Case "all"
            mbConfirmReAssign = True
            mbAssignAll = True
