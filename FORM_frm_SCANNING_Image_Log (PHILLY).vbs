...
Private Sub cmdValidate_Click()
    ...
    strImgPath = mstrLocalHoldPath & UCase(Replace(cnlyProvID, "_", "-"))
...
Private Sub CnlyClaimNum_AfterUpdate()
    ...
    If ImageType <> "PAC" Then
        ...
        Else
            ...
            cnlyProvID = Replace(Trim(rs("CnlyProvID")), " ", "")
    ...
    If Nz(ImageType, "") <> "" Then
        strSQL = "select 1 from v_SCANNING_image_log_tmp_TrackingNum  where ImageType = '" & ImageType & "'" & _
...
Private Sub Form_BeforeUpdate(Cancel As Integer)
    ...
    'the way it was, changed because of WPS claims dont have a letter to check against JS 10/11/2012
    
'    ' check received date can not be before MR request date
'    If IsDate(ReceivedDt) And Me.ImageType = "MR" Then
'        dLetterReqDt = DLookup("LetterReqDt", "v_SCANNING_Claim_Info", "CnlyClaimNum = '" & Me.CnlyClaimNum & "'")
...
'    End If
    ...
    Dim IsPRP As Boolean
    
    strReviewType = Nz(DLookup("Adj_ReviewType", "AUDITCLM_HDR", "Cnlyclaimnum = '" & Me.CnlyClaimNum & "'"), "")
    
    IsPRP = False
    If strReviewType = "PRP" Then
        IsPRP = True
    Else
        IsPRP = False
    End If

    If IsDate(ReceivedDt) And Me.ImageType = "MR" And Not IsPRP Then
        dLetterReqDt = Nz(DLookup("LetterReqDt", "v_SCANNING_Claim_Info", "CnlyClaimNum = '" & Me.CnlyClaimNum & "'"), #1/1/1900#)
        If ReceivedDt < dLetterReqDt Then
            MsgBox "Received date can not be prior to medical record request date"
            Cancel = True
        End If
    ...
    If IsDate(ReceivedDt) And Me.ImageType = "RECON" And left(strReviewType, 1) = "C" Then
        If Nz(DLookup("CnlyClaimNum", "v_SCANNING_Image_Log", "CnlyClaimNum = '" & Me.CnlyClaimNum & "' and ImageType = 'MR'"), "") = "" Then
            MsgBox "Warning!" & vbNewLine & vbNewLine & "ImageType should not be a RECON because this claim has not received an MR first." & vbNewLine & vbNewLine & "Please Verify!"
        End If
    End If
    ...
    cmd.CommandText = "dbo.usp_SCANNING_CheckandLog_InactivePayer_1"
    
    cmd.commandType = adCmdStoredProc
    cmd.Parameters.Refresh
    cmd.Parameters("@pUserID") = Identity.UserName()
    cmd.Parameters("@pSessionID") = 0
    cmd.Parameters("@pReceivedMeth") = ReceivedMeth
    cmd.Parameters("@pReceivedDt") = ReceivedDt
    cmd.Parameters("@pRequestNum") = 0
    cmd.Parameters("@pProvNum") = ProvNum
    cmd.Parameters("@pCnlyClaimNum") = CnlyClaimNum
    cmd.Parameters("@pImageType") = ImageType
...
Private Sub Form_Load()
    ...
    'JS 04/29/2013 Creating unique index every time in case table looses the index
    Call CreatePK("v_SCANNING_image_log_tmp_TrackingNum", "ScannedDt, CnlyClaimNum")
    
    Me.SortGroup = 1                ' default for listing by provider number
    
    ' Master = 1; Philly Office = 2; Humana office = 3
    iFormType = FormType.PhillyOffice
    'iFormType = FormType.Master
    Select Case iFormType
        Case 1              ' Master version
            Me.ScannedOffice = 1
            Me.RecordSource = "select * from v_SCANNING_image_log_tmp_TrackingNum  where AccountID = " & gintAccountID
        Case 2              ' Philly office
            Me.ScannedOffice = 1
            Me.ScannedOffice.Locked = True
            Me.RecordSource = "select * from v_SCANNING_image_log_tmp_TrackingNum  where ScanOperator = '" & Identity.UserName & "' and AccountID = " & gintAccountID
...
Private Sub ReceivedDt_Exit(Cancel As Integer)
    ...
    If ReceivedDt.Value <> "" Then
        If IsDate(ReceivedDt.Value) Then
            ...
            If ReceivedDt > Date Then
                ReceivedDt.DefaultValue = ""
                MsgBox Chr(34) & ReceivedDt.Value & Chr(34) & " is a future date." & vbCrLf & "Please enter a valid received date"
                Cancel = True
                GoTo OutOfProc
            End If
            
            If DateDiff("d", ReceivedDt, Date) > 10 Then
                MsgBox Chr(34) & ReceivedDt.Value & Chr(34) & " is older than 10 days." & vbCrLf & "Please make sure date is valid"
'                Cancel = True
'                GoTo OutOfProc
            End If
    ...
    If txtProvNum.ListIndex >= 0 Then
        Select Case SortGroup
            Case 1
                ...
                strRequestNum = txtProvNum.Column(4)
            Case 2
            ...
            Case 5
                ...
                strRequestNum = txtProvNum.Column(0)
        End Select
            ...
            cmd.ActiveConnection = myCode_ADO.CurrentConnection
            cmd.CommandText = "dbo.usp_SCANNING_CheckandLog_InactivePayer"
            
            cmd.commandType = adCmdStoredProc
            cmd.Parameters.Refresh
            cmd.Parameters("@pRequestNum") = strRequestNum
            cmd.Parameters("@pUserID") = Identity.UserName()
            cmd.Parameters("@pSessionID") = 0
            cmd.Parameters("@pERRMsg") = ""
...
Private Sub GetImageName(strImageName As String)
    ...
    If Me.ImageName & "" = "" Then
        Me.ImageName = UCase(Replace(Me.cnlyProvID, "_", "-")) & ImageType & "-" & Format(ScannedDt, "yyyymmddhhmmss")
    End If
