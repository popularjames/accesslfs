Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Compare Database
Option Explicit

Const CstrFrmAppID As String = "LetterQueueMain"

Private ciTabToLoad As Integer
Private ciBatchToSelect As Integer


Public Property Get ContractId() As Integer
    ContractId = Nz(Me.cmbContract, 0)
End Property



Public Property Get frmAppID() As String
    frmAppID = CstrFrmAppID
End Property

Private Property Get ClassName() As String
    ClassName = TypeName(Me)
End Property

Public Property Get LoadNextTab() As Integer
    LoadNextTab = ciTabToLoad
End Property
Public Property Let LoadNextTab(iLoadNextTab As Integer)
    ciTabToLoad = iLoadNextTab
    Me.TimerInterval = 500
End Property

Public Property Get BatchIdToSelect() As Integer
    BatchIdToSelect = ciBatchToSelect
End Property
Public Property Let BatchIdToSelect(iBatchIdToSelect As Integer)
    ciBatchToSelect = iBatchIdToSelect
End Property



Private Sub cmbContract_Change()
On Error GoTo Block_Err
Static lOldContractId As Integer
Dim strProcName As String
Dim oFormSubForm As Form_frm_LETTER_PrintQueue
Dim oFormSubFormAdd As Form_frm_LETTER_AddToQueue
' Form_Load

    strProcName = ClassName & ".cmbContract_Change"
    
    ' if it's the same as the last time then don't do anything
    
    If lOldContractId = Me.ContractId Then
        GoTo Block_Exit
    End If
    
    Select Case TypeName(Me.sub_form.Form)
    Case "Form_frm_LETTER_PrintQueue"

        Debug.Print Nz(Me.cmbContract, "")
        
        Set oFormSubForm = Me.sub_form.Form
        oFormSubForm.SelectedContractId = Me.ContractId
        oFormSubForm.RefreshMain
      
    Case "Form_frm_LETTER_AddToQueue"
        Debug.Print Nz(Me.cmbContract, "")
        
        Set oFormSubFormAdd = Me.sub_form.Form
'        oFormSubFormAdd.SelectedContractId = Me.ContractId

'        oFormSubFormAdd.Form_Load

    End Select
    
    lOldContractId = Me.ContractId
  
  
Block_Exit:
    Set oFormSubForm = Nothing
    Set oFormSubFormAdd = Nothing
    Exit Sub
Block_Err:
    ReportError Err, strProcName
    GoTo Block_Exit
End Sub

Private Sub cmdPrintFromFolderTool_Click()
    DoCmd.OpenForm "frm_LETTER_Legacy_Print_Tool", acNormal, , , acFormEdit, acWindowNormal
End Sub

Private Sub cmdProvReconciliationTool_Click()
    DoCmd.OpenForm "frm_LETTER_Reconciliation_Tool", acNormal, , , acFormEdit, acWindowNormal
End Sub

Private Sub Command23_Click()
Stop
End Sub

Private Sub Form_Close()
    Me.sub_form.SourceObject = Me.sub_form.Tag
End Sub


Private Sub Form_Load()
Dim iAppPermission As Integer
Dim sSupervisor As String
    
    Me.Caption = "Letter Maintenance"
    
    Call Account_Check(Me)
    iAppPermission = UserAccess_Check(Me)
    If iAppPermission = 0 Then
        Stop
        Me.visible = False
        DoCmd.Close acForm, Me.Name
        Exit Sub
    End If
    
'    '' Ok, if we get here and we are the 'Scanning" supervisorid then
'    '' open the new BOLD dashboard
'    '' Which we should have a button on it to open the old version..
'    '' so, if there are openargs then continue on - otherwise go for the new form
' Temporarilly "turn off" the new letter tool

'    If Nz(Me.OpenArgs, "") = "" Then
'    'If 1 = 2 Then
'        sSupervisor = Identity.UserSupervisorId()
'        Select Case UCase(sSupervisor)
'        Case "ADMIN", "DATA CENTER", "MANAGEMENT", "SCANNING"
'
'            DoCmd.OpenForm "frm_BOLD_Ops_Dashboard", acNormal
'            Me.visible = False
'            DoCmd.Close acForm, Me.Name
'            Exit Sub
'        End Select
'    End If
'
    Call LoadContractDetail

    lstAppPanel.RowSource = GetListBoxSQL(Me.Name)
    lstAppPanel.Requery
    Me.sub_form.SourceObject = Me.sub_form.Tag
    Me.sub_form.visible = False
    Me.lblSubAppTitle.visible = False
End Sub


'Private Sub Form_Resize()
''    ResizeControls Me.Form
'End Sub

Private Sub Form_Timer()
Dim vItem As Variant
Dim iItmCnt As Integer

    Me.TimerInterval = 0
    iItmCnt = Me.LoadNextTab
    
    If iItmCnt <> 0 Then
        If iItmCnt <= Me.lstAppPanel.ListCount Then
            Me.lstAppPanel.Selected(iItmCnt - 1) = True
'            Stop
            Me.lstAppPanel = Me.lstAppPanel.ItemData(iItmCnt - 1)
        End If
        ' now call the function...
        
        Call lstAppPanel_Click
    End If
    
End Sub



Private Sub lstAppPanel_Click()
Dim rs As DAO.RecordSet
Dim oFrm As Form_frm_LETTER_PrintLabels

    Set rs = CurrentDb.OpenRecordSet(GetListBoxRowSQL(lstAppPanel, Me.Name), dbOpenSnapshot, dbSeeChanges)
    If Not (rs.BOF And rs.EOF) Then
        Me.sub_form.SourceObject = ""
        lblSubAppTitle.Caption = rs("TabName")
        Me.sub_form.SourceObject = rs("FormName")
        Me.sub_form.visible = True
        Me.lblSubAppTitle.visible = True
        
        If Me.BatchIdToSelect <> 0 Then
            If UCase(rs("TabName")) = UCase("03 Print Labels") Then
                Set oFrm = Me.sub_form.Form
                oFrm.BatchToSelect = BatchIdToSelect
                
                Me.BatchIdToSelect = 0  ' reset..
            End If
        End If
        
    Else
        MsgBox "Application form has not been defined"
    End If
End Sub


Private Function LoadContractDetail() As Boolean
On Error GoTo Block_Err
Dim strProcName As String
Dim oAdo As clsADO
Dim oRs As ADODB.RecordSet
Dim oCbo As ComboBox
Dim sFirstVal As String

    strProcName = ClassName & ".LoadContractDetail"


    Set oAdo = New clsADO
    With oAdo
        .ConnectionString = GetConnectString("v_Code_Database")
        .SQLTextType = StoredProc
        .sqlString = "usp_XREF_Contract_Selection"
        .Parameters.Refresh
        .Parameters("@pAccountId") = gintAccountID
        .Parameters("@pAllowAll") = 0   ' nope.. one contract at a time for generating letters
                                        ' fight me on this !
        Set oRs = .ExecuteRS
        If Nz(.Parameters("@pErrMsg").Value, "") <> "" Then
            LogMessage strProcName, "ERROR", "An error occurred: " & .Parameters("@pErrMsg").Value
            GoTo Block_Exit
        End If
    End With
    
    Set oCbo = Me.cmbContract
    oCbo.RowSourceType = "Value List"
    oCbo.ColumnCount = 2
    oCbo.ColumnWidths = "0;.5;2;"
    oCbo.RowSource = "" ' clear it first..
    
    
    While Not oRs.EOF
        If sFirstVal = "" Then
            sFirstVal = oRs("ContractId").Value
        End If
        oCbo.AddItem oRs("ContractId").Value & ";" & oRs("ContractAbbrev").Value & ";" & oRs("ContractDesc").Value
        oRs.MoveNext
    Wend
    oCbo = sFirstVal
    
Block_Exit:
    Set oCbo = Nothing
    If Not oRs Is Nothing Then
        If oRs.State = adStateOpen Then oRs.Close
        Set oRs = Nothing
    End If
    Set oAdo = Nothing
    Exit Function
Block_Err:
    ReportError Err, strProcName
    GoTo Block_Exit
End Function
