...
'                Format(dtThrouDt, "mm-dd-yyyy") & "# " & ") OR (Status = 'G' AND LetterType <> 'REMINDADR' ))"
            sWhere = " WHERE ((Status = 'P' AND ProcessedDt >= #" & Nz(txtFromDate.Value, "01/01/1900") & "# and ProcessedDt < #" & _
                Format(dtThrouDt, "mm-dd-yyyy") & "# " & ") OR (Status = 'G' ))"
...
'''    If oLetterInst.IsDuplexJob = True Then
'''        ' here we need to put the code in to see if we need to add a blank page (the back page of the last document)
'''        ' but if we aren't going to combine the letters into a single document anymore then we don't really need to do this
'''        ' so for now, I'm going to forget it.
'''    End If
...
Dim lContractId As Long
    ...
    strSQL = "SELECT LetterType, TemplateLoc FROM LETTER_Type WHERE AccountID = " & gintAccountID & " and LetterType = '" & strLetterType & "' " & _
